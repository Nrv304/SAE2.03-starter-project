...
FeaturedMovies.formatOne = function (movie, favorites, profileId) {
    ...
    movieHtml = movieHtml.replace("{{duration}}", movie.duration);

    const isFavorite = favorites.some(fav => fav.id === movie.id);
    const favoriteButton = isFavorite
        ? `<button disabled>Favori</button>`
        : `<button class="add-to-favorites-button" onclick="C.addFavorites(${movie.id}, ${profileId})">
